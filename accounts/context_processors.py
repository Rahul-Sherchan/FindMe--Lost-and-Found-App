...
def notification_counts(request):
    ...
    if request.user.is_authenticated:
        ...
        # Add pending redeem requests count — used in admin sidebar badge
        # Only loaded for admin users to avoid unnecessary queries
        if request.user.is_staff or request.user.user_type == 'admin':
            try:
                from rewards.models import RedeemRequest
                context['pending_redeem_count'] = RedeemRequest.objects.filter(status='pending').count()
            except Exception:
                context['pending_redeem_count'] = 0
        else:
            context['pending_redeem_count'] = 0
